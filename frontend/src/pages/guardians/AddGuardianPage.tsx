...
import { createGuardian } from '../../store/slices/guardiansSlice';
import {
...
import { Save, ArrowBack, Person, Email, Phone, Work, Badge } from '@mui/icons-material';
...
export default function AddGuardianPage() {
    ...
    const { loading } = useSelector((state: RootState) => state.guardians);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!result.type.endsWith('/rejected')) {
            setSnackbar({ open: true, message: 'Guardian created successfully!', severity: 'success' });
            setTimeout(() => navigate('/guardians'), 1500);
        } else {
            setSnackbar({ open: true, message: 'Failed to create guardian', severity: 'error' });
        }
    };
    ...
    return (
        <Box>
            <Breadcrumbs sx={{ mb: 2 }}>
                ...
                <Typography color="text.primary" fontWeight={500}>Add Guardian</Typography>
            </Breadcrumbs>

            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                <Box>
                    <Typography variant="h4" fontWeight={700}>Add New Guardian</Typography>
                    <Typography variant="body2" color="text.secondary" mt={0.5}>Fill in the details to add a new guardian</Typography>
                </Box>
                        ...
                        <Card sx={{ boxShadow: '0 2px 12px rgba(0,0,0,0.06)', border: '1px solid rgba(0,0,0,0.06)', position: 'sticky', top: 90 }}>
                            <CardContent sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', py: 4 }}>
                                ...
                                <Typography variant="h6" fontWeight={600} textAlign="center">
                                    {form.first_name || form.last_name ? `${form.first_name} ${form.last_name}`.trim() : 'Guardian Name'}
                                </Typography>
                                <Typography variant="body2" color="text.secondary" mb={2} sx={{ textTransform: 'capitalize' }}>{form.relationship_type}</Typography>
                    ...
                    <Grid size={{ xs: 12, md: 8 }}>
                        ...
                        <Paper sx={{ p: 3, mb: 3, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', border: '1px solid rgba(0,0,0,0.06)' }}>
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid size={{ xs: 12, sm: 6 }}>
                                    <TextField label="Annual Income" fullWidth value={form.annual_income} onChange={handleChange('annual_income')} placeholder="e.g. ₹5,00,000" />
                                </Grid>
                            </Grid>
                        </Paper>
