...
class TimetableService:
    ...
    def _timetable_to_detail(self, t: Timetable) -> dict:
        return {
            ...
            "name": t.name,
            "status": t.status,
            "periods": [self._period_to_dict(p) for p in t.periods]
            if t.periods
            else [],
            "total_periods": len(t.periods) if t.periods else 0,
    ...
    def get_timetable(self, timetable_id: int, tenant_id: str) -> Optional[dict]:
        t = (
            ...
            .filter(Timetable.id == timetable_id, Timetable.tenant_id == tenant_id)
            .first()
        )
    ...
    def create_timetable(self, data: TimetableCreate, tenant_id: str) -> Timetable:
        ...
        if existing:
            raise ValueError(
                "A timetable already exists for this class and academic year"
            )
    ...
    def update_timetable(
        self, timetable_id: int, data: TimetableUpdate, tenant_id: str
    ) -> Optional[Timetable]:
        t = (
            self.db.query(Timetable)
            .filter(Timetable.id == timetable_id, Timetable.tenant_id == tenant_id)
    ...
    def add_period(
        self, timetable_id: int, data: PeriodCreate, tenant_id: str
    ) -> Period:
        t = (
            self.db.query(Timetable)
            .filter(Timetable.id == timetable_id, Timetable.tenant_id == tenant_id)
    ...
    def add_periods_bulk(
        self, timetable_id: int, periods: List[PeriodCreate], tenant_id: str
    ) -> List[Period]:
        t = (
            self.db.query(Timetable)
            .filter(Timetable.id == timetable_id, Timetable.tenant_id == tenant_id)
        ...
        for data in periods:
            p = Period(
                **data.model_dump(), timetable_id=timetable_id, tenant_id=tenant_id
            )
            self.db.add(p)
            result.append(p)
        self.db.commit()
    ...
    def update_period(
        self, period_id: int, data: PeriodUpdate, tenant_id: str
    ) -> Optional[Period]:
        p = (
            self.db.query(Period)
            .filter(Period.id == period_id, Period.tenant_id == tenant_id)
            .first()
        )
        if not p:
            return None
    ...
    def check_conflicts(self, timetable_id: int, tenant_id: str) -> List[dict]:
        ...
        my_periods = (
            self.db.query(Period).filter(Period.timetable_id == timetable_id).all()
        )
        ...
        other_periods = (
            self.db.query(Period)
            .filter(Period.timetable_id.in_(other_period_ids))
            .all()
        )
        ...
        for my_p in my_periods:
            ...
            for other_p in other_periods:
                ...
                if not self._times_overlap(
                    my_p.start_time, my_p.end_time, other_p.start_time, other_p.end_time
                ):
                    continue
                ...
                if (
                    my_p.teacher_id
                    and other_p.teacher_id
                    and my_p.teacher_id == other_p.teacher_id
                ):
                    teacher = (
                        self.db.query(Teacher)
                        .filter(Teacher.id == my_p.teacher_id)
                        .first()
                    )
                    other_tt = (
                        self.db.query(Timetable)
                        .filter(Timetable.id == other_p.timetable_id)
                        .first()
                    )
                    other_class_name = (
                        other_tt.class_ref.name
                        if other_tt and other_tt.class_ref
                        else "Unknown"
                    )
                    conflicts.append(
                        {
