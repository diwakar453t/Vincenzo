...
class TransportService:
    ...
    def update_route(self, route_id: int, data: dict, tenant_id: str):
        route = (
            ...
            .filter(
                TransportRoute.id == route_id, TransportRoute.tenant_id == tenant_id
            )
            .first()
        )
        if not route:
    ...
    def _route_dict(self, r: TransportRoute) -> dict:
        vehicle = (
            self.db.query(Vehicle).filter(Vehicle.id == r.vehicle_id).first()
            if r.vehicle_id
            else None
        )
        return {
    ...
    def get_vehicles(
        self, tenant_id: str, vehicle_type: str = None, status: str = None
    ):
    ...
    def _vehicle_dict(self, v: Vehicle) -> dict:
        assigned_routes = (
            self.db.query(TransportRoute)
            .filter(TransportRoute.vehicle_id == v.id)
            .count()
        )
        return {
            "id": v.id,
            ...
            if hasattr(v.vehicle_type, "value")
            else str(v.vehicle_type),
            "make": v.make,
            "model": v.model,
            "year": v.year,
            "capacity": v.capacity,
    ...
    def assign_student(self, data: dict, tenant_id: str):
        ...
        assign_date = (
            date.fromisoformat(data["assignment_date"])
    ...
    def cancel_assignment(self, assign_id: int, data: dict, tenant_id: str):
        assignment = (
            self.db.query(TransportAssignment)
            .filter(
                TransportAssignment.id == assign_id,
                TransportAssignment.tenant_id == tenant_id,
            )
            .first()
        )
        ...
        route = (
            self.db.query(TransportRoute)
            .filter(TransportRoute.id == assignment.route_id)
            .first()
        )
    ...
    def get_assignments(
        ...
        q = self.db.query(TransportAssignment).filter(
            TransportAssignment.tenant_id == tenant_id
        )
        if route_id:
    ...
    def _assignment_dict(self, a: TransportAssignment) -> dict:
        route = (
            self.db.query(TransportRoute)
            .filter(TransportRoute.id == a.route_id)
            .first()
        )
    ...
    def update_assignment_fee(
        self, assign_id: int, monthly_fee: float, fee_paid_till: str, tenant_id: str
    ):
        assignment = (
            self.db.query(TransportAssignment)
            .filter(
    ...
    def get_stats(self, tenant_id: str):
        routes = (
            self.db.query(TransportRoute)
            .filter(TransportRoute.tenant_id == tenant_id, TransportRoute.is_active)
            .all()
        )
        vehicles = (
            self.db.query(Vehicle)
            .filter(Vehicle.tenant_id == tenant_id, Vehicle.is_active)
            .count()
        )
        active_assigns = (
            self.db.query(TransportAssignment)
        ...
        total_revenue = (
            self.db.query(func.sum(TransportAssignment.monthly_fee))
            .filter(
                TransportAssignment.tenant_id == tenant_id,
                TransportAssignment.status == AssignmentStatus.active,
            )
            ...
            breakdown.append(
                {
                    ...
                    "occupancy_rate": round(
                        (r.current_students / r.max_capacity * 100)
                        if r.max_capacity > 0
                        else 0,
                        1,
                    ),
