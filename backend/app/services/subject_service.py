...
class SubjectService:
    ...
    def get_subject_by_code(self, code: str, tenant_id: str) -> Optional[Subject]:
        ...
        return (
            self.db.query(Subject)
            .filter(Subject.code == code, Subject.tenant_id == tenant_id)
            .first()
        )
    ...
    def create_subject(self, subject_data: SubjectCreate, tenant_id: str) -> Subject:
        ...
        subject = Subject(**subject_data.model_dump(), tenant_id=tenant_id)
    ...
    def update_subject(
        self, subject_id: int, subject_data: SubjectUpdate, tenant_id: str
    ) -> Optional[Subject]:
        ...
        if subject_data.code and subject_data.code != subject.code:
            ...
            if existing:
                raise ValueError(
                    f"Subject with code {subject_data.code} already exists"
                )
    ...
    def delete_subject(self, subject_id: int, tenant_id: str) -> bool:
        ...
        if assignment_count > 0:
            raise ValueError(
                "Cannot delete subject that is assigned to classes. Unassign it first."
            )
    ...
    def get_groups(self, tenant_id: str) -> List[SubjectGroup]:
        ...
        return (
            self.db.query(SubjectGroup)
            .filter(SubjectGroup.tenant_id == tenant_id)
    ...
    def get_group(self, group_id: int, tenant_id: str) -> Optional[SubjectGroup]:
        ...
        return (
            self.db.query(SubjectGroup)
            .filter(SubjectGroup.id == group_id, SubjectGroup.tenant_id == tenant_id)
            .first()
        )
    ...
    def update_group(
        self, group_id: int, data: SubjectGroupUpdate, tenant_id: str
    ...
    def delete_group(self, group_id: int, tenant_id: str) -> bool:
        ...
        self.db.query(Subject).filter(
            Subject.group_id == group_id, Subject.tenant_id == tenant_id
    ...
    def get_group_subject_count(self, group_id: int, tenant_id: str) -> int:
        ...
        return (
            self.db.query(Subject)
            .filter(Subject.group_id == group_id, Subject.tenant_id == tenant_id)
            .count()
        )
    ...
    def unassign_subject_from_class(
        self, subject_id: int, class_id: int, tenant_id: str
    ) -> bool:
    ...
    def get_class_subjects(self, class_id: int, tenant_id: str) -> list:
        ...
        assignments = (
            self.db.query(ClassSubject)
            .filter(
                ClassSubject.class_id == class_id, ClassSubject.tenant_id == tenant_id
            )
            .all()
        )
