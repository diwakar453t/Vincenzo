...
class GradeService:
    ...
    def create_category(
        self, data: GradeCategoryCreate, tenant_id: str
    ) -> GradeCategory:
    ...
    def update_category(
        self, cat_id: int, data: GradeCategoryUpdate, tenant_id: str
    ) -> Optional[GradeCategory]:
        c = (
            self.db.query(GradeCategory)
            .filter(GradeCategory.id == cat_id, GradeCategory.tenant_id == tenant_id)
            .first()
        )
        if not c:
            return None
    ...
    def create_grade(self, data: GradeCreate, tenant_id: str) -> Grade:
        percentage = (
            (data.marks_obtained / data.max_marks * 100) if data.max_marks > 0 else 0
        )
    ...
    def update_grade(
        self, grade_id: int, data: GradeUpdate, tenant_id: str
    ) -> Optional[Grade]:
        g = (
            self.db.query(Grade)
            .filter(Grade.id == grade_id, Grade.tenant_id == tenant_id)
            .first()
        )
        if not g:
            return None
    ...
    def calculate_gpa(
        self, student_id: int, exam_id: int, tenant_id: str
    ) -> Optional[dict]:
        grades = (
            self.db.query(Grade)
            .filter(
    ...
    def generate_report_card(
        self, student_id: int, academic_year: str, tenant_id: str
    ) -> Optional[dict]:
        ...
        cum_pct = (
            sum(e["overall_percentage"] for e in exams_data) / len(exams_data)
            if exams_data
            else 0
        )
        cum_gpa = (
            sum(e["gpa"] for e in exams_data) / len(exams_data) if exams_data else 0
        )
        overall_result = (
            "Pass" if all(e["result"] == "Pass" for e in exams_data) else "Fail"
        )
        ...
        return {
            ...
            "class_name": first_grade.class_ref.name
            if first_grade and first_grade.class_ref
            else None,
