...
class HostelService:
    ...
    def get_hostels(
        self, tenant_id: str, hostel_type: str = None, is_active: bool = None
    ):
    ...
    def update_hostel(self, hostel_id: int, data: dict, tenant_id: str):
        hostel = (
            ...
            .filter(Hostel.id == hostel_id, Hostel.tenant_id == tenant_id)
            .first()
        )
        if not hostel:
    ...
    def create_room(self, data: dict, tenant_id: str):
        hostel = (
            self.db.query(Hostel)
            .filter(Hostel.id == data["hostel_id"], Hostel.tenant_id == tenant_id)
            .first()
        )
        if not hostel:
            raise ValueError("Hostel not found")
    ...
    def update_room(self, room_id: int, data: dict, tenant_id: str):
        room = (
            self.db.query(HostelRoom)
            .filter(HostelRoom.id == room_id, HostelRoom.tenant_id == tenant_id)
            .first()
        )
        if not room:
    ...
    def _room_dict(self, r: HostelRoom) -> dict:
        ...
        return {
            ...
            if hasattr(r.room_type, "value")
            else str(r.room_type),
            "capacity": r.capacity,
            "occupied": r.occupied,
    ...
    def allocate_room(self, data: dict, tenant_id: str):
        room = (
            self.db.query(HostelRoom)
            .filter(HostelRoom.id == data["room_id"], HostelRoom.tenant_id == tenant_id)
            .first()
        )
        if not room:
            raise ValueError("Room not found")
        ...
        alloc_date = (
            date.fromisoformat(data["allocation_date"])
            if data.get("allocation_date")
            else date.today()
        )
        exp_vacate = (
            date.fromisoformat(data["expected_vacating_date"])
            if data.get("expected_vacating_date")
            else None
        )
        ...
        alloc = RoomAllocation(
            ...
            expected_vacating_date=exp_vacate,
    ...
    def vacate_room(self, alloc_id: int, data: dict, tenant_id: str):
        ...
        vacate_date = (
            date.fromisoformat(data["vacating_date"])
            if data.get("vacating_date")
            else date.today()
        )
    ...
    def get_allocations(
        ...
        total = q.count()
        allocs = (
            q.order_by(RoomAllocation.allocation_date.desc())
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def _alloc_dict(self, a: RoomAllocation) -> dict:
        ...
        return {
            ...
            "expected_vacating_date": str(a.expected_vacating_date)
            if a.expected_vacating_date
            else None,
    ...
    def get_availability(self, tenant_id: str, hostel_id: int = None):
        ...
        return [self._room_dict(r) for r in rooms if r.occupied < r.capacity], len(
            [r for r in rooms if r.occupied < r.capacity]
        )
    ...
    def update_allocation_fee(
        self, alloc_id: int, monthly_fee: float, fee_paid_till: str, tenant_id: str
    ):
        alloc = (
            self.db.query(RoomAllocation)
            .filter(
    ...
    def get_stats(self, tenant_id: str):
        hostels = (
            self.db.query(Hostel)
            .filter(Hostel.tenant_id == tenant_id, Hostel.is_active)
            .all()
        )
        ...
        total_revenue = (
            self.db.query(func.sum(RoomAllocation.monthly_fee))
            .filter(
                RoomAllocation.tenant_id == tenant_id,
                RoomAllocation.status == AllocationStatus.active,
            )
            ...
            breakdown.append(
                {
                    ...
                    "occupancy_rate": round(
                        (h.occupied_beds / h.total_beds * 100)
                        if h.total_beds > 0
                        else 0,
                        1,
                    ),
        ...
        return {
            ...
            "occupancy_rate": round(
                (occupied / total_beds * 100) if total_beds > 0 else 0, 1
            ),
