...
class GuardianService:
    ...
    def get_guardian(self, guardian_id: int, tenant_id: str) -> Optional[Guardian]:
        ...
        return (
            ...
            .filter(Guardian.id == guardian_id, Guardian.tenant_id == tenant_id)
            .first()
        )

    def create_guardian(
        self, guardian_data: GuardianCreate, tenant_id: str
    ) -> Guardian:
        ...
        if student_ids:
            students = (
                ...
                .filter(Student.id.in_(student_ids), Student.tenant_id == tenant_id)
                .all()
            )
    ...
    def update_guardian(
        self, guardian_id: int, guardian_data: GuardianUpdate, tenant_id: str
    ) -> Optional[Guardian]:
        ...
        update_data = guardian_data.model_dump(
            exclude_unset=True, exclude={"student_ids"}
        )
