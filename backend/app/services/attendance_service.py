...
class AttendanceService:
    ...
    def get_student_attendance(
        ...
        total = q.count()
        items = (
            q.order_by(StudentAttendance.date.desc()).offset(skip).limit(limit).all()
        )
    ...
    def bulk_mark_student_attendance(
        self, data: StudentAttendanceBulk, tenant_id: str
    ) -> List:
        results = []
    ...
    def bulk_mark_staff_attendance(
        self, data: StaffAttendanceBulk, tenant_id: str
    ) -> List:
        results = []
        for entry in data.entries:
    ...
    def student_attendance_stats(
        self, student_id: int, tenant_id: str, academic_year: Optional[str] = None
    ) -> dict:
        q = self.db.query(StudentAttendance).filter(
            StudentAttendance.tenant_id == tenant_id,
    ...
    def class_daily_report(
        self, class_id: int, target_date: date, tenant_id: str
    ) -> dict:
        cls = (
            self.db.query(Class)
        ...
        pct = (
            round(stats["present"] / total_students * 100, 1)
            if total_students > 0
            else 0
        )
        return {
    ...
    def student_monthly_view(
        self, student_id: int, month: int, year: int, tenant_id: str
    ) -> dict:
        ...
        by_date = {
            r.date: r.status.value if hasattr(r.status, "value") else r.status
            for r in records
        }
        days = [
            {
                "date": date(year, month, d + 1),
                "status": by_date.get(date(year, month, d + 1)),
            }
    ...
    def staff_monthly_view(
        self, teacher_id: int, month: int, year: int, tenant_id: str
    ) -> dict:
    ...
    def class_attendance_report(
        self, class_id: int, tenant_id: str, academic_year: Optional[str] = None
    ) -> List[dict]:
