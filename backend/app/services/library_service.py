...
class LibraryService:
    ...
    def create_book(self, data: dict, tenant_id: str):
        book = Book(
            **data, tenant_id=tenant_id, available_copies=data.get("total_copies", 1)
        )
        self.db.add(book)
        self.db.commit()
    ...
    def update_book(self, book_id: int, data: dict, tenant_id: str):
        book = (
            self.db.query(Book)
            .filter(Book.id == book_id, Book.tenant_id == tenant_id)
            .first()
        )
        if not book:
    ...
    def update_member(self, member_id: int, data: dict, tenant_id: str):
        member = (
            self.db.query(LibraryMember)
            .filter(LibraryMember.id == member_id, LibraryMember.tenant_id == tenant_id)
            .first()
        )
        if not member:
    ...
    def issue_book(self, data: dict, tenant_id: str):
        book = (
            self.db.query(Book)
            .filter(Book.id == data["book_id"], Book.tenant_id == tenant_id)
            .first()
        )
        if not book:
            raise ValueError("Book not found")
        ...
        member = (
            self.db.query(LibraryMember)
            .filter(
                LibraryMember.id == data["member_id"],
        ...
        if member.books_issued >= member.max_books_allowed:
            raise ValueError(
                f"Member has reached max limit ({member.max_books_allowed} books)"
            )
    ...
    def return_book(self, issue_id: int, data: dict, tenant_id: str):
        issue = (
            self.db.query(IssueReturn)
            .filter(IssueReturn.id == issue_id, IssueReturn.tenant_id == tenant_id)
            .first()
        )
        if not issue:
        ...
        member = (
            self.db.query(LibraryMember)
            .filter(LibraryMember.id == issue.member_id)
            .first()
        )
    ...
    def get_issues(
        ...
        total = q.count()
        issues = (
            q.order_by(IssueReturn.issue_date.desc()).offset(skip).limit(limit).all()
        )
    ...
    def calculate_fine(self, issue_id: int, tenant_id: str):
        ...
        if today > issue.due_date:
            ...
            return {
                "fine_amount": days * issue.fine_per_day,
                "fine_paid": False,
                "days_overdue": days,
            }
    ...
    def get_stats(self, tenant_id: str):
        total_books = (
            self.db.query(Book)
            .filter(Book.tenant_id == tenant_id, Book.is_active)
            .count()
        )
        total_copies = (
            self.db.query(func.sum(Book.total_copies))
            .filter(Book.tenant_id == tenant_id, Book.is_active)
            .scalar()
            or 0
        )
        available = (
            self.db.query(func.sum(Book.available_copies))
            .filter(Book.tenant_id == tenant_id, Book.is_active)
            .scalar()
            or 0
        )
        total_members = (
            self.db.query(LibraryMember)
            .filter(LibraryMember.tenant_id == tenant_id)
            .count()
        )
        active_members = (
            self.db.query(LibraryMember)
            .filter(
                LibraryMember.tenant_id == tenant_id,
                LibraryMember.status == MembershipStatus.active,
            )
            .count()
        )
        ...
        total_returned = (
            ...
            .filter(
                IssueReturn.tenant_id == tenant_id,
                IssueReturn.status == IssueStatus.returned,
            )
            .count()
        )
        total_overdue = (
            ...
            .filter(
                IssueReturn.tenant_id == tenant_id,
                IssueReturn.status == IssueStatus.overdue,
            )
            .count()
        )

        total_fines = (
            self.db.query(func.sum(IssueReturn.fine_amount))
            .filter(IssueReturn.tenant_id == tenant_id)
            .scalar()
            or 0
        )
        fines_collected = (
            self.db.query(func.sum(IssueReturn.fine_amount))
            .filter(IssueReturn.tenant_id == tenant_id, IssueReturn.fine_paid)
            .scalar()
            or 0
        )
        ...
        # Category breakdown
        cats = (
            self.db.query(Book.category, func.count(Book.id))
            .filter(
    ...
    def _issue_dict(self, i: IssueReturn) -> dict:
        ...
        member = (
            self.db.query(LibraryMember).filter(LibraryMember.id == i.member_id).first()
        )
        return {
            "id": i.id,
