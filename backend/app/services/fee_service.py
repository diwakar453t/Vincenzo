...
class FeeService:
    ...
    def update_fee_group(self, gid: int, data: dict, tenant_id: str):
        g = (
            self.db.query(FeeGroup)
            .filter(FeeGroup.id == gid, FeeGroup.tenant_id == tenant_id)
            .first()
        )
        if not g:
            return None
    ...
    def update_fee_type(self, tid: int, data: dict, tenant_id: str):
        t = (
            ...
            .filter(FeeType.id == tid, FeeType.tenant_id == tenant_id)
            .first()
        )
        if not t:
            return None
    ...
    def assign_fees(self, data: FeeAssignRequest, tenant_id: str) -> list:
        ...
        for r in results:
            if not isinstance(r, dict):
                self.db.refresh(r)
        return [
            self._assignment_dict(r) if not isinstance(r, dict) else r for r in results
        ]
    ...
    def collect_fee(self, data: FeeCollectionCreate, tenant_id: str):
        ...
        if data.assignment_id:
            a = (
                self.db.query(StudentFeeAssignment)
                .filter(StudentFeeAssignment.id == data.assignment_id)
                .first()
            )
            if a:
    ...
    def delete_collection(self, cid: int, tenant_id: str) -> bool:
        c = (
            ...
            .filter(FeeCollection.id == cid, FeeCollection.tenant_id == tenant_id)
            .first()
        )
        if not c:
            return False
        ...
        if c.assignment_id:
            a = (
                self.db.query(StudentFeeAssignment)
                .filter(StudentFeeAssignment.id == c.assignment_id)
                .first()
            )
            if a:
    ...
    def get_defaulters(self, tenant_id: str, class_id: Optional[int] = None) -> tuple:
        ...
        if class_id:
            q = q.join(Student, Student.id == StudentFeeAssignment.student_id).filter(
    ...
    def get_financial_summary(
        self, tenant_id: str, academic_year: str = "2025-26"
    ) -> dict:
        assignments = (
            self.db.query(StudentFeeAssignment)
            .filter(
        ...
        collections = (
            ...
            .filter(
                ...
                FeeCollection.academic_year == academic_year,
            )
            .all()
        )
        ...
        return {
            ...
            "collection_rate": round(
                (total_collected / total_assigned * 100) if total_assigned > 0 else 0, 1
            ),
            "payment_method_breakdown": method_breakdown,
            "monthly_collection": [
                {"month": k, "amount": v} for k, v in sorted(monthly.items())
            ],
        }
    ...
    def get_receipt(self, collection_id: int, tenant_id: str) -> Optional[dict]:
        c = (
            self.db.query(FeeCollection)
            .filter(
                FeeCollection.id == collection_id, FeeCollection.tenant_id == tenant_id
            )
            .first()
        )
        if not c:
