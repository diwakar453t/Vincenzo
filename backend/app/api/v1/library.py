...
def create_book(
    ...
    _require_admin(user)
    return BookResponse(
        **LibraryService(db).create_book(data.model_dump(), user.tenant_id)
    )
...
def update_book(
    ...
    try:
        return BookResponse(
            **LibraryService(db).update_book(
                book_id, data.model_dump(exclude_unset=True), user.tenant_id
            )
        )
...
def list_members(
    ...
    user = _get_user(current_user, db)
    members, total = LibraryService(db).get_members(
        user.tenant_id,
    ...
    return MemberListResponse(
        members=[MemberResponse(**m) for m in members], total=total
    )
...
def create_member(
    ...
    return MemberResponse(
        **LibraryService(db).create_member(data.model_dump(), user.tenant_id)
    )
...
def update_member(
    ...
    try:
        return MemberResponse(
            **LibraryService(db).update_member(
                mid, data.model_dump(exclude_unset=True), user.tenant_id
            )
        )
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
...
def issue_book(
    ...
    try:
        return IssueReturnResponse(
            **LibraryService(db).issue_book(data.model_dump(), user.tenant_id)
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def return_book(
    ...
    try:
        return IssueReturnResponse(
            **LibraryService(db).return_book(
                issue_id, data.model_dump(), user.tenant_id
            )
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def list_issues(
    ...
    user = _get_user(current_user, db)
    issues, total = LibraryService(db).get_issues(
        user.tenant_id,
        member_id=member_id,
        book_id=book_id,
        status=status,
    ...
    return IssueReturnListResponse(
        issues=[IssueReturnResponse(**i) for i in issues], total=total
    )
...
def pay_fine(
    ...
    try:
        return IssueReturnResponse(
            **LibraryService(db).pay_fine(issue_id, user.tenant_id)
        )
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
