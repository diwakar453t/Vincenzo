...
def initiate_payment(
    ...
    return PaymentService(db).initiate_payment(
        user.tenant_id, user.id, data.model_dump()
    )
...
def verify_payment(
    ...
    result = PaymentService(db).verify_payment(
        user.tenant_id, data.order_id, data.payment_id, data.signature
    )
...
def list_transactions(
    ...
    user = _get_user(current_user, db)
    result = PaymentService(db).list_transactions(
        user.tenant_id,
        status=status,
...
def payment_stats(
    ...
    data["recent_transactions"] = [
        PaymentTransactionResponse(**t) for t in data["recent_transactions"]
    ]
...
def get_transaction(
    ...
    return PaymentTransactionResponse(
        **PaymentService(db).get_transaction(txn_id, user.tenant_id)
    )
...
def initiate_refund(
    ...
    user = _get_user(current_user, db)
    result = PaymentService(db).initiate_refund(
        data.transaction_id, user.tenant_id, data.amount, data.reason
    )
    return PaymentTransactionResponse(**result)
