...
def generate_report(
    ...
    try:
        ...
        data = ReportService(db).generate_report(
            req.report_type, user.tenant_id, **kwargs
        )
...
def export_csv(
    ...
    try:
        ...
        return StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename={req.report_type}_report.csv"
            },
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def list_saved_reports(
    ...
    user = _get_user(current_user, db)
    reports, total = ReportService(db).get_saved_reports(
        user.tenant_id, report_type=report_type
    )
    return ReportListResponse(
        reports=[ReportResponse(**r) for r in reports], total=total
    )
