...
def get_school_settings(
    ...
    return SchoolSettingsResponse(
        **SettingsService(db).get_school_settings(user.tenant_id)
    )
...
def update_school_settings(
    ...
    user = _get_user(current_user, db)
    result = SettingsService(db).update_school_settings(
        user.tenant_id, data.model_dump(exclude_none=True)
    )
...
def update_academic_year(
    ...
    user = _get_user(current_user, db)
    result = SettingsService(db).update_academic_year(
        year_id, user.tenant_id, data.model_dump(exclude_none=True)
    )
    return AcademicYearResponse(**result)
...
def bulk_update_preferences(
    ...
    user = _get_user(current_user, db)
    results = SettingsService(db).bulk_update_preferences(
        user.tenant_id, [p.model_dump() for p in data.preferences]
    )
