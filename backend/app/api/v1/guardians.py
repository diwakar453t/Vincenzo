...
def list_guardians(
    ...
    return GuardianListResponse(
        guardians=guardian_items, total=total, skip=skip, limit=limit
    )
...
def create_guardian(
    ...
    if not user or user.role not in ["admin", "teacher"]:
        raise HTTPException(
            status_code=403, detail="Only admins or teachers can create guardians"
        )
...
def update_guardian(
    ...
    if not user or user.role not in ["admin", "teacher"]:
        raise HTTPException(
            status_code=403, detail="Only admins or teachers can update guardians"
        )
...
def link_student(
    ...
    if not user or user.role not in ["admin", "teacher"]:
        raise HTTPException(
            status_code=403, detail="Only admins or teachers can link students"
        )
...
def unlink_student(
    ...
    if not user or user.role not in ["admin", "teacher"]:
        raise HTTPException(
            status_code=403, detail="Only admins or teachers can unlink students"
        )
