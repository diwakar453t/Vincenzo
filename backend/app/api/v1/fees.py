...
def list_fee_groups(
    ...
    return FeeGroupListResponse(
        fee_groups=[FeeGroupResponse(**g) for g in items], total=total
    )


@router.post(
    "/groups", response_model=FeeGroupResponse, status_code=status.HTTP_201_CREATED
)
def create_fee_group(
    data: FeeGroupCreate,
    current_user: dict = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    user = _get_user(current_user, db)
    _require_admin(user)
    return FeeGroupResponse(
        **FeeService(db).create_fee_group(data.model_dump(), user.tenant_id)
    )


@router.put("/groups/{gid}", response_model=FeeGroupResponse)
def update_fee_group(
    gid: int,
    data: FeeGroupUpdate,
    current_user: dict = Depends(get_current_user),
    db: Session = Depends(get_db),
    ...
    user = _get_user(current_user, db)
    _require_admin(user)
    result = FeeService(db).update_fee_group(
        gid, data.model_dump(exclude_unset=True), user.tenant_id
    )
    if not result:
...
def list_fee_types(
    ...
    user = _get_user(current_user, db)
    items, total = FeeService(db).get_fee_types(
        user.tenant_id, fee_group_id, class_id, is_active
    )
    return FeeTypeListResponse(
        fee_types=[FeeTypeResponse(**t) for t in items], total=total
    )


@router.post(
    "/types", response_model=FeeTypeResponse, status_code=status.HTTP_201_CREATED
)
def create_fee_type(
    data: FeeTypeCreate,
    current_user: dict = Depends(get_current_user),
    ...
    return FeeTypeResponse(
        **FeeService(db).create_fee_type(data.model_dump(), user.tenant_id)
    )
...
def update_fee_type(
    ...
    user = _get_user(current_user, db)
    _require_admin(user)
    result = FeeService(db).update_fee_type(
        tid, data.model_dump(exclude_unset=True), user.tenant_id
    )
    if not result:
...
@router.post(
    "/assignments",
    response_model=FeeAssignmentListResponse,
...
def assign_fees(
    ...
    try:
        ...
        return FeeAssignmentListResponse(
            assignments=[FeeAssignmentResponse(**a) for a in items], total=len(items)
        )
...
def list_assignments(
    ...
    user = _get_user(current_user, db)
    items, total = FeeService(db).get_assignments(
        user.tenant_id, student_id, fee_type_id, status_filter
    )
    return FeeAssignmentListResponse(
        assignments=[FeeAssignmentResponse(**a) for a in items], total=total
    )
...
def list_collections(
    ...
    user = _get_user(current_user, db)
    items, total = FeeService(db).get_collections(
        user.tenant_id, student_id, fee_type_id, from_date, to_date
    )
    return FeeCollectionListResponse(
        collections=[FeeCollectionResponse(**c) for c in items], total=total
    )
...
def get_defaulters(
    ...
    return DefaulterListResponse(
        defaulters=[DefaulterInfo(**d) for d in items], total=total
    )
