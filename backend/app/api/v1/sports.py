...
def list_sports(
    ...
    sports, total = SportsService(db).get_sports(
        user.tenant_id, category=category, status=status
    )
...
def create_sport(
    ...
    return SportResponse(
        **SportsService(db).create_sport(data.model_dump(), user.tenant_id)
    )
...
def update_sport(
    ...
    try:
        return SportResponse(
            **SportsService(db).update_sport(
                sport_id, data.model_dump(exclude_unset=True), user.tenant_id
            )
        )
...
def register_student(
    ...
    try:
        return ParticipationResponse(
            **SportsService(db).register_student(data.model_dump(), user.tenant_id)
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def list_participations(
    ...
    user = _get_user(current_user, db)
    parts, total = SportsService(db).get_participations(
        user.tenant_id, sport_id=sport_id, student_id=student_id, status=status
    )
    return ParticipationListResponse(
        participations=[ParticipationResponse(**p) for p in parts], total=total
    )
...
def update_participation(
    ...
    try:
        return ParticipationResponse(
            **SportsService(db).update_participation(
                part_id, data.model_dump(exclude_unset=True), user.tenant_id
            )
        )
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
...
def create_achievement(
    ...
    try:
        return AchievementResponse(
            **SportsService(db).create_achievement(data.model_dump(), user.tenant_id)
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def list_achievements(
    ...
    user = _get_user(current_user, db)
    achs, total = SportsService(db).get_achievements(
        user.tenant_id, sport_id=sport_id, student_id=student_id, level=level
    )
    return AchievementListResponse(
        achievements=[AchievementResponse(**a) for a in achs], total=total
    )
...
def update_achievement(
    ...
    try:
        return AchievementResponse(
            **SportsService(db).update_achievement(
                ach_id, data.model_dump(exclude_unset=True), user.tenant_id
            )
        )
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
