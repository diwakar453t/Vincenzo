...
def send_notification(
    ...
    result = NotificationService(db).send_notification(
        data.model_dump(), user.tenant_id, sender_id=user.id
    )
...
def broadcast_notification(
    ...
    count = NotificationService(db).broadcast(
        data.model_dump(), user.tenant_id, sender_id=user.id
    )
...
def unread_count(
    ...
    user = _get_user(current_user, db)
    _, _, unread = NotificationService(db).get_notifications(
        user.id, user.tenant_id, is_read=False, limit=0
    )
...
def notification_stats(
    ...
    return NotificationStats(
        **NotificationService(db).get_stats(user.id, user.tenant_id)
    )
...
def mark_read(
    ...
    user = _get_user(current_user, db)
    try:
        result = NotificationService(db).mark_as_read(
...
def update_preferences(
    ...
    pref = NotificationService(db).update_preferences(
        user.id, user.tenant_id, data.model_dump(exclude_unset=True)
    )
    return PreferenceResponse(**pref)
