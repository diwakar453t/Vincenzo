...
def list_hostels(
    ...
    hostels, total = HostelService(db).get_hostels(
        user.tenant_id, hostel_type=hostel_type, is_active=is_active
    )
    return HostelListResponse(
        hostels=[HostelResponse(**h) for h in hostels], total=total
    )
...
def create_hostel(
    ...
    return HostelResponse(
        **HostelService(db).create_hostel(data.model_dump(), user.tenant_id)
    )
...
def update_hostel(
    ...
    try:
        return HostelResponse(
            **HostelService(db).update_hostel(
                hostel_id, data.model_dump(exclude_unset=True), user.tenant_id
            )
        )
...
def list_rooms(
    ...
    user = _get_user(current_user, db)
    rooms, total = HostelService(db).get_rooms(
        user.tenant_id,
        hostel_id=hostel_id,
        status=status,
        room_type=room_type,
    ...
    return HostelRoomListResponse(
        rooms=[HostelRoomResponse(**r) for r in rooms], total=total
    )
...
def create_room(
    ...
    try:
        return HostelRoomResponse(
            **HostelService(db).create_room(data.model_dump(), user.tenant_id)
        )
    except ValueError as e:
...
def update_room(
    ...
    try:
        return HostelRoomResponse(
            **HostelService(db).update_room(
                room_id, data.model_dump(exclude_unset=True), user.tenant_id
            )
        )
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
...
def allocate_room(
    ...
    try:
        return AllocationResponse(
            **HostelService(db).allocate_room(data.model_dump(), user.tenant_id)
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def vacate_room(
    ...
    try:
        return AllocationResponse(
            **HostelService(db).vacate_room(alloc_id, data.model_dump(), user.tenant_id)
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def list_allocations(
    ...
    user = _get_user(current_user, db)
    allocs, total = HostelService(db).get_allocations(
        user.tenant_id,
        hostel_id=hostel_id,
        room_id=room_id,
    ...
    return AllocationListResponse(
        allocations=[AllocationResponse(**a) for a in allocs], total=total
    )
...
def room_availability(
    ...
    user = _get_user(current_user, db)
    rooms, total = HostelService(db).get_availability(
        user.tenant_id, hostel_id=hostel_id
    )
    return HostelRoomListResponse(
        rooms=[HostelRoomResponse(**r) for r in rooms], total=total
    )
...
def update_hostel_fees(
    ...
    try:
        return HostelResponse(
            **HostelService(db).update_hostel_fees(
                hostel_id, monthly_fee, user.tenant_id
            )
        )
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
...
def update_allocation_fee(
    ...
    try:
        return AllocationResponse(
            **HostelService(db).update_allocation_fee(
                alloc_id, monthly_fee, fee_paid_till, user.tenant_id
            )
        )
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
