...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "color", sa.String(length=10), nullable=False, server_default="#3D5EE1"
        ),
    ...
    op.create_index(
        op.f("ix_leave_applications_tenant_id"), "leave_applications", ["tenant_id"]
    )
    op.create_index("ix_leave_applications_status", "leave_applications", ["status"])
    op.create_index(
        "ix_leave_applications_dates", "leave_applications", ["start_date", "end_date"]
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_leave_applications_tenant_id"), table_name="leave_applications"
    )
