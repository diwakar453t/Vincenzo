...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "estimated_time_min", sa.Integer(), nullable=True, server_default="0"
        ),
    ...
    op.create_index(
        "ix_transport_assignments_tenant_id", "transport_assignments", ["tenant_id"]
    )
    op.create_index(
        "ix_transport_assignments_student_id", "transport_assignments", ["student_id"]
    )
    op.create_index(
        "ix_transport_assignments_route_id", "transport_assignments", ["route_id"]
    )
    op.create_index(
        "ix_transport_assignments_status", "transport_assignments", ["status"]
    )
